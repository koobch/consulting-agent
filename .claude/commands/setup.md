...
# [선택 — 도메인 특화] 아래는 도메인에 따라 추가
# export STEAM_API_KEY="your-key-here"          # 게임
# export IGDB_CLIENT_ID="your-id-here"          # 게임
# export IGDB_CLIENT_SECRET="your-secret-here"  # 게임
ENVEOF
```
...
**WHY**: 자바스크립트 렌더링이 필요한 동적 웹페이지를 크롤링. 앱스토어, Steam, IR 페이지 등 SPA 사이트의 데이터 수집에 필수.
...
| 옵션 | 설명 |
|------|------|
| **게임 산업** | 게임 개발사/퍼블리셔 리서치 (기본 프로파일 제공) |
| **직접 입력** | 다른 산업 (바이오, SaaS, 반도체 등) — 도메인 프로파일 스키마 기반으로 생성 |

### 게임 산업 선택 시
`knowledge/domain/` 디렉토리의 기본 파일이 이미 게임 산업용으로 작성되어 있으므로 확인만 한다:
```bash
ls knowledge/domain/
```
4개 파일(`domain-profile.md`, `industry-metrics.md`, `product-lifecycle-models.md`, `data-sources-domain.md`)이 존재하면 PASS.
...
도메인 특화 API (도메인 프로파일에 따라 다름):
| API | 도메인 | 무료 여부 | 발급 URL |
|-----|--------|----------|---------|
| **Steam Web API** | 게임 | 무료 | https://steamcommunity.com/dev/apikey |
| **IGDB** | 게임 | 무료 | https://api-docs.igdb.com/#getting-started |

> 추가 도메인 특화 API는 `knowledge/domain/data-sources-domain.md`를 참조한다.
