...
### §8-1. [도메인 특화 API — /setup Step 8.5에서 자동 추가]

> `/setup` 실행 시 Step 8.5에서 Claude가 해당 도메인의 공개 API를 웹 리서치하여 이 섹션에 추가한다.
> 리서치 실행 중 새로운 도메인 API를 발견하면 여기에 축적한다.

### §8-2. FRED API (미국 매크로 경제지표)

> **공통 API**: 모든 도메인 리서치에서 매크로 분석 시 사용.
